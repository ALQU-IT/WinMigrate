...
| Field | Meaning |
| --- | --- |
| `category` | `user_files`, `browser_profile`, `browser_passwords`, `wifi`, `printers`, `mapped_drives`, `env_vars`, `scheduled_tasks`, `fonts`, `outlook`, `notepad`, `displays`, `wallpaper`, `personalization`, `startup`, `dev_config`, `file_associations`, `software`, `office`. |
